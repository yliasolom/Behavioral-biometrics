...
subgraph "Построение прогнозных моделей"
  B --> C(Выбор модели)
  C --> D(Обучение модели)
  D --> E(Оценка модели)
end
...
subgraph "Оптимизация"
  E --> F(Тюнинг параметров)
  F --> G(Оптимизация)
end

subgraph "Тестирование"
  G --> I(Тестирование модели)
  I --> J(Оценка результатов)
end

subgraph "Закрытие технического долга"
  J --> K(Оптимизация кода)
  K --> L(Обработка ошибок)
end

subgraph "Подготовка пилота"
  L --> M(Подготовка данных пилота)
  M --> N(Тестирование на пилоте)
end

subgraph "Другие этапы"
  N --> O((Дополнительные шаги))
end
